...
async fn effective_suspended_account_ids(
    ...
    let observed_at: DateTime<Utc> =
        sqlx::query_scalar("SELECT clock_timestamp()").fetch_one(&mut *connection).await?;
    let suspended_ids: Vec<i64> = sqlx::query_scalar(
        "SELECT DISTINCT account_id FROM identity.sanctions \
         WHERE account_id = ANY($1) AND kind = 'suspend' AND revoked_at IS NULL \
    ...
    .await?;
    Ok(suspended_ids.into_iter().collect())
}
...
pub async fn find_mention_targets_by_handles_tx(
    ...
    let mut targets = sqlx::query_as::<_, MentionTarget>(
        "SELECT account.id, account.handle::text, \
                COALESCE(privacy.mention_policy, 'everyone') AS mention_policy \
         FROM identity.accounts AS account \
         LEFT JOIN identity.profile_privacy AS privacy ON privacy.account_id = account.id \
    ...
    let suspended_ids = effective_suspended_account_ids(connection, &target_ids).await?;
    targets.retain(|target| !suspended_ids.contains(&target.id));
    Ok(targets)
}
...
pub async fn lock_notification_recipient(
    ...
    if effective_suspended_account_ids(connection, &[account_id]).await?.contains(&account_id) {
        return Ok(None);
    }
...
pub async fn lock_active_interaction_accounts(
    ...
    Ok(effective_suspended_account_ids(connection, &expected_ids).await?.is_empty())
}
...
///
/// Eligibility requires an active account and no currently effective suspend
...
pub async fn is_credit_recipient_eligible(
    ...
    let is_active = sqlx::query_scalar::<_, bool>(
        "SELECT true FROM identity.accounts account \
         WHERE account.id = $1 AND account.status = 'active' FOR SHARE OF account",
    )
    .bind(account_id)
    .fetch_optional(&mut *connection)
    .await?
    ...
    Ok(!effective_suspended_account_ids(connection, &[account_id]).await?.contains(&account_id))
}
