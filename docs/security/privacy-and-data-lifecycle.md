...
- Selection raw import 只保存 snapshot hash/bytes/counts、验证结果和不指向自然人的 lowercase
  ...
  sync job 只保存发起账号 id、3–500 字符运维原因、account-scoped idempotency hash、lease/progress、bounded
  error code 和 count-only result。普通课程/选课客户端不可见；`operations.jobs` inventory 只返回 actor id 与
  job 状态，不返回 reason、idempotency hash、lease token 或 request fingerprint，数据库原始记录只供受控
  worker/DB operations。Reason 不得包含姓名、邮箱、token、原始课表、搜索词或其他 PII/secret。
...
Selection sync 的 `requested_by` 只引用同一 lifecycle account id，不复制 handle/email；operator 账号 purge
后该外键指向不可反查邮箱的 tombstone，直到 job 按 90/365 天规则清理。Reason 与 job history 属于运营/
治理目的，不能作为 owner-private projection 随请求立即改写；append-only governance actor history 继续按
其独立保留政策。Import 的 `imported_by` 必须是非个人 role/service label，因此账号删除不需要解析或改写
该字段；被替代 import evidence 由 365 天 housekeeping 清理，当前 materialization authority 不得提前删。
...
- Selection import/sync 是平台运营与投影完整性证据，不是发起 staff 的 owner content：普通 owner export
  不包含 `imported_by`、sync reason/idempotency hash、lease/progress 或 result。Capability-gated inventory
  是独立运维访问面，不能借 owner export 扩大披露。
...
- Selection sync 的 succeeded/cancelled job（含 actor/reason/hash/result）在 terminal 90 天后清理，dead job
  在 terminal 365 天后清理；worker 启动即执行并每日重试。Superseded `selection.import_runs` 在 imported
  365 天后清理，但当前最新一条 validated import 必须保留为 raw materialization authority，即使超过
  365 天；它只有在新 authority 成功写入后才进入 superseded 窗口。对应 append-only governance event 的
  独立保留期仍按 governance audit 政策，不因 job row 清理而提前删除。
