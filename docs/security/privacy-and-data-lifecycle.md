...
- Lifecycle worker 已覆盖数据库 owner-private projection、用户搜索 reconciliation，以及 lifecycle
  dead-letter 的 capability-gated list/requeue API；仍不存在通用 legal-hold registry、统一 operator UI
  或跨域逐项 reconciliation report。
- Bound public media 只进入现有 content-retention/GC grace，当前 worker 不直接删除 OSS object；实际
  scanner/orphan GC、CDN cache、日志和 backup expiry 仍未形成完整、可演练的删除闭环。
...
除 recovery/export 的当前时限外，其他具体天数仍为 `Decision needed`，必须分别定义：
