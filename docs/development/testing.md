...
Migration `0066` 必须覆盖同一 installation 再登录只替换该 session、不同 installation 共存、legacy
客户端 active session 上限、显式撤销的旧 refresh 不触发全账号 replay 处置，以及 rolling-window 旧
refresh writer 省略新列时由数据库继承 account-scoped digest。Web 同时验证 installation ID 在正常退出后
保留、清站点数据可重置，且旧标签页 refresh 失败不会删除另一标签页刚写入的新 token。
...
S3 的最低证据不是“SQL 能跑”：必须在隔离 fresh database 通过 sqlx ledger 到最新 migration，用 13 张
selection raw 与 3 张 legacy-course raw fixture 验证两份 snapshot-bound audit、materialize 重跑、
teaching-class identity、真实教师前缀、跨教师重复排课折叠、unknown week/location、历史 aggregate scope、
catalogue 人工数据保留与 community aggregate 不重复累加。真实快照验收再抽样至少 50 条 weekday/slot/week，通过公开 HTTP
结果反查 PostgreSQL 真值；Meilisearch 必须实测 code、教师、拼音、calendar/major/time filter 与 PG
rehydration，并清空一次 index 验证 readiness reconciliation，而不是只测序列化函数。
