...
>
> 最近核验：2026-07-15，`contract/openapi.yaml`、migrations `0060`–`0068` 与 owner-domain tests
...
Migration `0068` 将选课公开身份从课程代码收紧为具体教学班。`offeringId` 是
...
- `/selection/offerings` 使用有界、query-fingerprint 绑定的 opaque cursor；weekday、startSlot、endSlot
  必须同时出现且范围有序。时间筛选默认保留 `scheduleUnknown` 或 `weeksUnknown`，只有客户端显式传
  `includeUnknownSchedule=false` 才排除，避免把上游缺失误报为无冲突。
- 旧的 by-major/by-nature/by-code/search/timeslots 路由在至少一个完整 consumer 迁移窗口内保持 deprecated
  且硬上限返回；它们只给确定性的兼容代表，不再承诺课程代码等于教学班。删除这些路由必须另立
  contract change，并先证明 Web 与 Flutter 已不再消费。
...
fence，失败最多八次退避后进入 dead 状态；dead retry 的理由写 account audit。worker 仍随 API 进程启动，
所以独立 worker deployment 与跨实例 SLO 仍是后续平台工作。其他搜索索引路径仍有裸 `tokio::spawn`，
属于迁移目标，不是推荐的新模式。
