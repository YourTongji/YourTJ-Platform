...
>
> 最近核验：2026-07-13，`contract/openapi.yaml`、migrations `0060`–`0065` 与 owner-domain tests
...
- 当前 Forum attachment、Forum content author avatar 与 Promotion DTO 保留 expiry；public profile/account/
  relationship/search/DM avatar 兼容字段仍只返回 URL string。它们是需要 additive contract 或统一有界
  refetch 收敛的 `Partial` 例外，不能被复制到新 surface，也不能用更长 TTL/public origin 掩盖。
