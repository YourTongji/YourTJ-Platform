...
| 媒体 | `Partial` | 后端 STS、回调、审核、owner-only 状态查询及 clean avatar/banner/thread/comment binding 已存在；staff 待审证据使用一次性审计同源代理，Forum 有 exact AST/reference、versioned usage、delete/restore grace 和 Web 状态 UI。课评/私信仍无 binding，且缺 scanner/缩略图/EXIF/配额/实际 GC worker/CDN 策略 |
