...
| 领域 | 状态 | 已验证问题 |
|---|---|---|
| 登录注册 | `Partial` | 后端已完成 purpose-bound 一次性 code、密码防枚举和 reset/change 撤销语义；Web 仍只有混合验证码表单，未暴露密码登录/找回 |
| 账号与会话 | `Partial` | 后端 access 已绑定可撤销 session/auth version，并有设备列表/单个/其他/全部撤销接口；Web 无设备中心，且缺 onboarding、recent-auth、自助导出/停用/删除，refresh token 仍存 localStorage |
...
| 聚合搜索 | `Partial` | courses/reviews/threads 已由独立 search domain 返回 typed、可跳转且回表重验的结果，type/limit 生效；仍缺综合结果页、每类 cursor、users/boards/tags、highlight/纠错和可靠 outbox 更新 |
...
1. Web 拆分登录、注册、忘记/重置密码，接入现有 password/session API；提供设备中心并避免邮箱
   前缀成为公开 handle。
2. 主题/评论 canonical policy、评论 SQL、引用约束和事务边界已完成；下一步补草稿契约、显式
   pending、`contentVersion/expectedVersion` 和 durable side-effect outbox。
3. 为 Onebox 补维护中的 HTML parser、规范化/错误缓存与完整网络 fixture；现有逐跳
   scheme/allowlist/DNS/public-IP pin、流式 body 限制和禁用远程预览图继续作为安全基线。
4. 修正 trust promotion/demotion，并在发主题时执行 board lock/min-trust；明确 staff 内容达到普通
   举报阈值时自动隐藏还是升级复核，并为阈值与角色边界补测试。
5. 接通 tags、tag filter、subscription、poll/vote/viewer state、read tracking 与撤销互动。
6. 删除伪造等级/徽章/摘要和无行为按钮；界面只能展示后端真实能力。
7. 对齐通知 event preference、补齐所有 target URL、Web 实时消费和 multi-instance delivery。
8. 定义公告 revision/seen/ack，完成“该用户未看过公告”的全局提示/弹窗决策与实现。
9. 停止持久化任意头像/UGC 外链，确定 OSS asset/clean/binding；修正 block 会永久删除对象的后台文案。
10. 在已对齐的 typed 聚合搜索上补可靠 outbox/reconciliation、综合结果页与局部失败；新增
    users/boards/tags 前先落实 discoverability、block 和账号状态规则。
11. 为上述身份、通知、内容、搜索和公告行为补 handler→DB 与前端关键旅程验证。
12. 为 credit 增加只读 reconcile job/指标/告警；历史 constraint anomaly 走单独兼容决策，不在
