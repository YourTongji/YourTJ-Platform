...
| 选课与课表 | `Partial` | `jcourse-db-backup` 已通过受限 export/import 管线在隔离 PG 中完成真实投影、抽样和性能验证；`selection` 以 teaching-class `offeringId`、显式周次/地点未知标记、有界游标和时间筛选对外，所有集合按 calendar 隔离。Web/Flutter 以同一 identity 管理 client-owned 课表，覆盖 possible-only override、1–20 节、空闲筛选和本机 JSON 备份。全量物化、Meili/Redis 切换已进入带 heartbeat/generation fence 的 durable 幂等 job；Meili clear→add 仍非原子。上游仍无官方选中结果/可靠变更源，跨设备课表同步待决，浏览器与真机端到端 journey 仍缺 |
...
- 选课已完成 `teachingClassId`、`calendarId` 查询分区和 Web/Flutter 本机课表冲突语义；下一步是从
  上游快照保留 weeks/location/status，定义 catalogue↔teaching-class bridge，并补真实浏览器/设备旅程。
