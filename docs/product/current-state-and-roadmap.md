...
3. 主题/评论创建与编辑共享 canonical 内容验证/治理管线、评论更新 SQL、引用约束和写事务已完成；
   下一步补草稿契约、显式 pending、`contentVersion/expectedVersion` 和 durable side-effect outbox。
4. 为 Onebox 补维护中的 HTML parser、规范化/错误缓存与完整网络 fixture；现有逐跳
...
11. 在已对齐的 typed 聚合搜索上补可靠 outbox/reconciliation、综合结果页与局部失败；新增
    users/boards/tags 前先落实 discoverability、block 和账号状态规则。
12. 为上述身份、通知、内容、搜索和公告行为补 handler→DB 与前端关键旅程验证。
13. 为 credit 增加只读 reconcile job/指标/告警；历史 constraint anomaly 走单独兼容决策，不在
