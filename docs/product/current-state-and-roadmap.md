...
| 通知 | `Partial` | bounded cursor/unread/逐条与全部已读、安全站内 target、Web 角标/筛选/SSE 回源刷新已接通；偏好 key 与事件类型仍不一致，部分旧事件无 target，也没有 multi-instance delivery |
...
6. 对齐通知 event preference、补齐所有 target URL 和 multi-instance delivery。
