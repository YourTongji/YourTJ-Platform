...
>
> 最近核验：2026-07-12，migrations `0053`–`0062`、OpenAPI/Web 与 deploy workflows
...
| 个人资料 | `Partial` | display name、bio、HTTPS website、clean + published avatar/banner reference、owner 上传/状态恢复/绑定 UI、关系数和 profile/list/DM/discoverability/activity/mention 隐私已落地；Profile 提供 `canViewActivity`。仍缺 handle history、公开 media/likes tabs，且 public profile/account/relationship/search/DM avatar 兼容字段尚未携带签名 URL expiry/统一错误刷新 |
...
| 媒体 | `Partial` | 代码已交付 private Ingest → bounded decode/metadata stripping → private Delivery 三变体 → 五分钟 signed CDN 的 publication pipeline；owner pending preview、ADMIN 受控自审、Forum/Promotion typed projection、processing retry 和 block 后 ordered cleanup 均有测试。Profile/list avatar 兼容 DTO 的 expiry/refetch 尚未统一；通用 GC 默认关闭，目标环境双 bucket/RAM/CDN/secrets、真实 smoke 与 inventory reconciliation 尚需签字；课评/私信无 binding，file/PDF scanner 与 legal hold 仍待决 |
...
6. 任意头像 URL 已停写，avatar/banner/thread/comment/promotion 已完成 private Ingest、owner/status
   recovery、clean + published binding、sanitized variants 与 owning-domain signed Delivery；下一步先按
   runbook 完成 staging 双 bucket/CDN/rotation/reconciliation，再接课评/私信和 file/PDF scanner。后台
   只通过一次性审计同源代理预览，不展示 object key/hash/vendor URL；block 立即停止签名并持久执行 purge/
   Delivery/Ingest cleanup。
...
- `plain_v1/markdown_v1` 契约、安全 renderer、编辑器、预览、CAS autosave、已发布内容冲突恢复和
  clean + published Forum 图片已完成；下一步补跨客户端 conformance corpus、file/PDF scanner、provider
  配置验收与真实浏览器旅程。
