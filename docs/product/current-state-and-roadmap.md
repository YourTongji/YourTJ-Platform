...
>
> 最近核验：2026-07-17，全系统文档差距审查、Credit 生命周期与 Web wallet safety
...
- 积分 ledger 有 signing intent、hash chain、数据库 append-only 防护；tip 与 escrow 状态机有
  target/party 校验、事务锁和 CAS。`/wallet` 返回 Identity owner projection 的 active public key；Web
  使用 exact signing bytes、环境/账号隔离的 non-extractable key 与请求前 durable pending reconciliation。
...
| 账号与会话 | `Partial` | session-bound access、refresh replay、防 stale-password recent-auth、同源 installation 摘要下的重复登录 session replacement/30 活跃上限、focused onboarding、recovery-only credential、30 天停用/删除恢复、八域 owner export/purge、durable worker 与 Web 自助入口已完成；Credit export 已覆盖 owner task/product/purchase 私密投影，purge 清空被删除账号拥有的 contact/delivery 与未消费 intent 而保留账本/交易事实。仍缺完整跨标签 refresh single-flight、versioned policy publish、handle history、legal-hold/备份/OSS 全闭环，refresh token 仍存 localStorage，JWT signing key rotation/多 key 验证尚未交付 |
...
| 创作体验 | `Partial` | 主题/评论已持久化显式 content format，Web 已接 CodeMirror 编辑/预览、安全 renderer、debounced 云端草稿、account-scoped 7 天 IndexedDB 恢复副本和 clean + published 图片插入；Flutter 也已接安全 renderer、创作预览、云端草稿/冲突与上传，backend/Web/Flutter 共同消费恶意样例 corpus。Owner pending preview/processing 状态可恢复；历史 iOS corpus 和跨端真实 journey 仍缺 |
| Link preview/Onebox | `Current` | HTTPS 抓取边界已有逐跳 allowlist/DNS/public-IP pin、禁用代理、精确 HTML MIME/UTF-8、流式 body 上限、HTML5 parser、规范化 URL、无远程预览图、版本化成功/失败 cache 和不访问公网的受控 TLS fixture；Web 仅在用户显式打开对话框并确认后调用 `/onebox`，只显示安全文字字段，再次确认才插入规范化 Markdown 链接 |
...
| 选课与课表 | `Partial` | `jcourse-db-backup` 的 16 张 selection/legacy-course 表已通过受限 import 管线在隔离 PG 中完成真实投影、评分 aggregate 和性能复验；`selection` 以 teaching-class `offeringId`、显式周次/地点未知标记、历史评分 scope、有界游标和时间筛选对外，所有集合按 calendar 隔离。Web/Flutter 以同一 identity 管理 client-owned 课表并显示真实评分参考，覆盖 possible-only override、1–20 节、空闲筛选和本机 JSON 备份。Catalogue/selection search 有数据库 generation/readiness、启动与周期恢复，未 ready 时 fail closed；durable job 的 Meili clear→add 仍非原子。上游仍无官方选中结果/可靠变更源，历史正文/身份未迁移，跨设备课表同步待决，浏览器与真机端到端 journey 仍缺 |
...
| 私信 | `Partial` | canonical 1:1、DM policy、单条陌生请求、incoming/sent 请求箱、accept/decline/withdraw/report、独立 unread/request 角标、archive/delete/recover、搜索、mute、durable 多实例刷新提示和最小举报证据已接通。普通消息用 sender-scoped client UUID 串行幂等，Web/Flutter 发送中气泡保留原 UUID 显式重试/丢弃；仍缺附件、request expiry、typing/presence 及 retention/legal-hold worker |
...
| 积分运营 | `Partial` | 用户侧 verify、内容打赏和 escrow 完整性已加固；Identity active public key owner projection、Web 环境/账号隔离 non-extractable key、legacy seed 精确迁移、单账号 challenge replacement/rate limit、one-shot neutral claim proof、已认领 review 迁移凭据清理、跨标签 operation claim 和 Web/Flutter lock-aware intent outcome 已接通。持久化只读 reconcile、单并发/幂等执行、逐钱包漂移指标、独立 capability、审计和管理视图已完成；仍缺密钥丢失/轮换恢复、未认领 legacy authority 截止、intent quota/retention、告警/SLO 与受审批 projection 重建，历史 constraint anomaly 需单独兼容策略 |
...
下表是 2026-07-17 审查中已识别但**未在本轮实现**的缺口，状态为 `Partial` 或 `Decision needed`，不得据此宣称
`Current`。编号用于后续 Issue/PR 引用；只有对应实现、负向测试和 canonical docs 一起落地后才能关闭。

| 编号 | 状态 | 已识别缺口与收口条件 |
|---|---|---|
| `SYS-AUDIT-01` | `Partial` | Credit signing intent 缺 account-scoped active/retained quota 和过期清理；task/product/contact/delivery 输入缺统一 OpenAPI + handler 长度上限，amount/balance 也未明确限制为 JSON safe integer 或改用 decimal string，SQL bigint 边界缺 overflow 回归。需补事实源限额、清理 worker 与数值/滥用边界。 |
| `SYS-AUDIT-02` | `Partial` | 无认证的 `/wallet/ledger/verify` 每次扫描公开全账本，成本随 ledger 单调增长且缺 rate limit/checkpoint。需给公开验证定义有界证明或受控缓存，并保留完整离线/operator verification。 |
| `SYS-AUDIT-03` | `Partial` | 通知/outbox retention worker 启动后先等待完整 24 小时，首次清理延迟不可观测。需启动即执行一次、继续周期调度，并覆盖失败重试与 retention 指标。 |
| `SYS-AUDIT-04` | `Partial` | badge pending mint bridge 以无 claim/lease 的查询后处理，多实例可能并发处理同一 row；ledger idempotency 不能替代 worker fencing，且 `minted_at` 更新失败被忽略。需 owner-domain claim/fencing、幂等终态与并发回归。 |
| `SYS-AUDIT-05` | `Partial` | Web 展示的课程“AI 点评摘要”仍来自 backend placeholder；`updatedAt` 实际返回 RFC3339 字符串而 OpenAPI/Web 声明 Unix integer。需真实 owner model/空状态、秒级契约和生成类型回归。 |
| `SYS-AUDIT-06` | `Partial` | Flutter 多个 profile/search/forum surface 仍消费 legacy `avatarUrl` compatibility 字段，缺统一 expiry 和 refetch 语义。需迁移 typed Media delivery，并覆盖到期/加载失败/环境切换。 |
| `SYS-AUDIT-07` | `Partial` | PR preview workflow 使用 `cancel-in-progress`，远端 deploy/cutover/cleanup 没有跨 run host lock；新提交或 close cleanup 可与旧 SSH 步骤交错。需 per-PR remote fencing、可中断阶段边界和竞态测试。 |
| `SYS-AUDIT-08` | `Partial` | 本地 quickstart 的 media 示例只预填 CDN URL/slot/TTL，却留空同一完整配置组，可能触发 startup fail-closed；Compose 数据服务与默认 API 又绑定所有接口。需一致的 disabled/synthetic media profile，并把本机端口显式限制到 loopback。 |
| `SYS-AUDIT-09` | `Partial` | CI docs-impact 只强制 contract/migration/workflow/少量配置，未覆盖一般 `ops/deploy/**`；PR preview path filter 也未覆盖全部部署输入。需集中路径分类及单测，确保运维行为变化必带 canonical docs 且触发相应 CI。 |
| `SYS-AUDIT-10` | `Partial` | 新建 task/product hold 已不再把自由文本 title 复制进 ledger/intent proof，但切换前的 append-only ledger 可能仍含 title。需做历史数据盘点、法律/保留决策与对外披露；禁止为了清理而改写 hash chain。 |
| `SYS-AUDIT-11` | `Decision needed` | 账号请求删除后立即无法认证 Credit，但已存在的 in-progress/submitted task 和 pending/accepted/delivered purchase 没有删除前 preflight 或 durable settlement，可永久卡住 escrow。本轮只防止 inactive/suspended counterpart 新 accept/purchase；仍需产品/合规决定“删除前必须结清”、受控自动 cancel/refund 或 lifecycle settlement worker，并覆盖各 party/status/并发。 |
...
2. Onebox 的维护中 HTML5 parser、规范化 URL、版本化缓存、受控 HTTPS 网络 fixture 和 Web 显式
   opt-in 文字预览旅程已完成；逐跳 scheme/allowlist/DNS/public-IP pin、禁用代理、精确 MIME/charset、
   流式 body 限制和禁用远程预览图继续作为安全基线。只有产品决定展示图片时才引入 media proxy。
...
- Flutter 已按 typed API/secure session/账号隔离、semantic theme、自适应 shell，再到普通用户与
  capability 管理旅程的依赖顺序接入真实服务；下一步不是继续堆页面，而是先补公告历史、头像 expiry
  等 contract/read-side 缺口，再完成 golden、真实环境 integration、Android/iOS device、
...
- 选课已完成 `teachingClassId`、`calendarId` 查询分区、真实 weeks/location、历史评分 scope、搜索 readiness
  和 Web/Flutter 本机课表冲突语义；下一步是取得可靠 status/变更源、定义 catalogue↔teaching-class bridge，
  并补真实浏览器/设备旅程。
  跨设备课表在明确 server owner、导出/删除和冲突合并语义前仍是 `Decision needed`。
...
- `plain_v1/markdown_v1` 契约、安全 renderer、编辑器、预览、CAS autosave、已发布内容冲突恢复和
  clean + published Forum 图片已完成；backend/Web/Flutter 已消费共享 conformance corpus，Web 另有
  7 天 account-scoped IndexedDB 恢复副本。下一步补历史 iOS consumer、file/PDF scanner、provider
  配置验收与真实 browser/device 旅程。
- 现有 latest/hot/subscription/following feed 和 typed 搜索已补 Web 跨列表乐观互动、canonical 分享、
  游标续页、搜索 debounce/取消/键盘导航、安全 canonical 高亮与保守拼写建议；下一步完成拼音/别名和
  可靠索引更新。
- DM policy、单消息请求状态机、archive/delete/recover 和 client UUID 幂等重试已完成；下一步补
  private attachment、request expiry 与 retention/legal-hold worker。
