...
| 徽章与认证 | `Partial` | 成就徽章、人工身份/特殊认证和实时角色标识已经拆分；人工认证具备独立 capability、typed definition、可到期/撤销 grant、私有 evidence reference、同事务审计、后台 UI 与公开 profile 安全投影。成就定义/人工授予/撤销 UI、认证通知和证据存储/复核政策仍缺 |
...
- 推广 asset usage/GC 与效果聚合、成就徽章运营后台、认证通知/证据复核、typed settings、durable job center。
