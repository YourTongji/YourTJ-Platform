...
>
> 最近核验：2026-07-15，migrations `0053`–`0068`、OpenAPI/Web、Flutter parity audit 与 CI workflows
...
| Feed | `Partial` | latest/hot/subscriptions/following 已拆分真实后端语义，following 基于 user_follows 并执行账号/内容/block/mute 过滤；首页、论坛、Profile 与收藏使用同一 query-key/viewer-state 家族，vote/bookmark 在相关 cache 间乐观更新、失败回滚并最终回源，SSE 只作当前账号的定向失效提示。筛选同步 URL，canonical 分享、灯箱、稳定续页与匿名登录回跳已接通；仍缺透明的 recommended 规则 |
| 聚合搜索 | `Partial` | courses/reviews/threads/users/boards/tags 已由独立 search domain 返回 typed、可跳转且回表重验的结果；type/limit、query+scope 绑定的有界 cursor、all→单类“查看更多”、局部失败保真、安全 canonical 字符区间高亮、保守拼写建议和 Web 综合页已生效。全局搜索框有 400ms debounce、旧请求取消、键盘导航与确定的课评 deep link；仍缺拼音/别名与可靠 outbox 更新 |
| 课程/选课/本机课表 | `Partial` | 选课镜像已以 `teachingClassId` 作为详情、节次和本机去重标识；年级、专业、性质和搜索均强制 `calendarId`，搜索候选按 owner 当前事实回表并保持相关性顺序。Web/Flutter 本机课表按 environment+account/anonymous+calendar 分区；可解析周次有交集才确认冲突，无交集不冲突，缺失/无法解析为可能冲突并需显式覆盖。仍缺 catalogue↔teaching-class typed bridge、物化后周次/教室/状态事实、可靠索引 reconciliation、跨设备同步和真实 browser/device journey；本机课表不是正式选课结果 |
| 评课 | `Partial` | 公开列表和精确详情均由 Reviews owner 回表重验可见性；服务端返回 `viewerLiked/canEdit/canReport`，Web/Flutter 支持 like/unlike、本人编辑、非本人举报与搜索精确 deep link，匿名和用户态 cache 隔离。仍缺 typed reviewer avatar、历史身份映射、课程合并/归档后的 deep-link 保留政策与真实设备旅程 |
| 通知 | `Partial` | 论坛/互动/关注/DM/成就/认证 producer 与业务事实同事务写 0054 outbox，consumer 有 lease/SKIP LOCKED、幂等 receipt、聚合、当前 privacy/prefs/content 与可撤销 source generation 重验、自动重试/dead-letter、理由化人工重试；target、Redis 多实例 SSE hint、账号/受限凭证分区 Web cache 已完成。Web 使用与真实卡片结构匹配的 skeleton，并按用户本地自然日分组普通通知。治理通知继续使用同处置事务 private store；仍缺 digest delivery status/retry、dead-letter 告警/SLO 与外部渠道运营验证 |
...
| Flutter Android/iOS | `Partial` | `mobile/` 已作为 proprietary clean-room 工程进入 monorepo；固定 Dart generator/typed client、自适应五项 shell、secure session、账号/环境隔离、共享 Markdown/Ed25519/OSS signing vectors 已落库。身份、首页/推广、论坛/媒体、课程/评课/教学班级别的本机课表、搜索/资料、通知/全局公告、私信、申诉/生命周期、钱包以及 capability 管理中心都已消费真实 API；19 项矩阵仍缺不同程度的后端契约、读侧 UX、golden/integration/device、hosted link association、release signing 和商店发布证据，因此均为 `Partial`，不能整体宣称 Web parity |
