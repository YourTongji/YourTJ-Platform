...
| 个人资料 | `Partial` | display name、bio、HTTPS website、clean OSS avatar/banner reference、关系数和 profile/list/DM/discoverability 隐私已落地；仍缺 handle history、activity/mention privacy、上传器和公开 media/likes tabs |
...
| 创作体验 | `Partial` | 主题/评论已持久化显式 content format，Web 已接 CodeMirror 编辑/预览与安全 renderer，服务端有 AST profile 和纯文本投影；仍缺 autosave、clean asset 图片、跨客户端 conformance、草稿对齐与编辑冲突处理 |
...
| 媒体 | `Partial` | 后端 STS、回调、审核及 owner+clean avatar/banner binding 已存在，任意头像 URL 已停写；帖子、评论、课评、私信仍无 binding，且缺 scanner/缩略图/EXIF/配额/孤儿回收/CDN 策略 |
...
6. 任意头像 URL 已停写且 avatar/banner binding 已建立；下一步把上传 UI 接到资料和 UGC，完成
   scanner/variants/GC/CDN，并保持后台如实说明 block 会永久删除对象。
...
- Display name、bio、banner、受控链接和 OSS 头像 binding 已落地；下一步补 handle history/cooldown、
  scanner/variants、前端上传器和 orphan GC。
