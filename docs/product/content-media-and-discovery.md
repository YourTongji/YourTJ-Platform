...
- 当前数据库有 intent、`pending/clean/blocked`、profile reference binding，以及 Forum 专用的
  version-aware `asset_usages`。自动 scan、跨所有业务的 usage 和实际 GC worker 仍为 `Planned`，不能
  因主题/评论链路存在声称完整 OSS 产品链路。
