...
- Feed/论坛列表的首张 clean attachment 与详情 Markdown 图片可通过独立按钮打开共享灯箱；卡片导航链接
  不包裹按钮，避免 `a > button`。详情页按 canonical attachment 顺序切换多图；链接图片保持链接语义，
  不叠加灯箱。灯箱支持关闭按钮、空白区、Escape、左右方向键，并在关闭后把焦点还给触发图片。
