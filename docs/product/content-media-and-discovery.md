...
- 通用上传组件尚未绑定到头像/主题/评论等最终用户旅程；头像仍接受外部 URL，上传记录也没有绑定到
...
当前主题/评论的 plain-text 输入已经统一执行规范化、长度/结构、watched words 与引用约束，
canonical row/revision 和主题附属数据也具备事务边界；asset binding、Markdown AST 解析、显式
pending 状态、乐观版本和 durable outbox 仍是该目标管线的缺口。
