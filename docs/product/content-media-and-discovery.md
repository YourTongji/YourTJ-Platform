...
>
> 最近核验：2026-07-12，`contract/openapi.yaml`、migration `0061` 与 owner-domain tests
...
- Media 后端已有 account-bound upload intent、短期 OSS STS、回调验签、MIME/大小限制和
  ...
  `unpublished/processing/published/failed/blocked` publication 状态。Admin queue 在 cursor/limit 前按
  strict role hierarchy 过滤；同一审核员完成可信 raster preview 后才能 approve，file/PDF 在 scanner
  前 fail closed。Approve 只排队处理，不等于已公开。
...
- 头像/封面设置已挂接上述链路，并按持久化 usage 查询 owner-safe 上传状态；pending owner 预览走
  同源鉴权 blob，Web 只在 `clean + published` 后允许绑定，并区分 pending、processing、failed、blocked。
  解除绑定不直接删除原始资产。
...
- Forum attachment 与 Promotion 已保留 Delivery expiry/刷新语义；public profile/account/relationship/
  search/DM avatar 的兼容 DTO 仍只有 URL string，尚未统一 typed expiry 或 image-error refetch。该 gap
  不允许通过延长签名 TTL、改 public-read 或恢复 direct OSS URL 规避。
...
- Raster 的受限解码、metadata stripping 与变体生成已交付；内容安全自动扫描仍未交付，因此 raster
  image 必须由人工可信预览后批准，file/PDF 不允许人工绕过 scanner 直接放行。
