...
- Markdown 第一阶段已在主题/评论上线，但仍缺 autosave、clean asset 图片插入/绑定、跨 Web/iOS/
  Flutter conformance corpus，以及评论/课评/公告各自更细的 syntax profile。
- queued 内容目前复用 hidden 状态，没有独立 pending 状态/审核任务；编辑没有
  `contentVersion/expectedVersion` 乐观冲突检测，搜索/通知等事务外副作用也没有 durable outbox。
- drafts 的 OpenAPI、Rust response 和 Web 仍未完整统一；互动 DTO 仍缺服务端 `canEdit`、
  `canDelete`、`canModerate` 能力字段。
- 首页已移除固定摘要、按 index 伪造徽章和无行为的收藏/分享/筛选/频道按钮；但首页 feed 仍没有
  canonical 正文摘要、列表级 viewer state、用户 following 或推荐模型。
- Avatar/banner 后端只接受本人 clean OSS asset，任意头像 URL 已停写；通用上传组件仍未挂到资料
  设置，主题/评论等 UGC 也没有 asset binding，因此“基础上传成功”不等于媒体产品闭环。
...
canonical row/content format/revision 和主题附属数据也具备事务边界；asset binding、显式 pending
状态、乐观版本和 durable outbox 仍是该目标管线的缺口。
...
- 编辑器 debounce autosave，显示保存中/已保存/失败/离线状态，离开时提示未同步内容。
- drafts list/get/save/delete 的 OpenAPI、Rust 响应和 Web 必须统一。
...
- 当前数据库只有 intent 与 `pending/clean/blocked`，没有自动 scan、asset binding 或 GC；这些均为
  `Planned`，不能因字段存在声称完整 OSS 产品链路。
