...
显式内容 pending 状态和 durable outbox 仍是该目标管线的缺口。
...
- 当前数据库有 intent、`pending/clean/quarantined/blocked`、moderation evidence、durable provider
  deletion job、profile reference binding，以及 Forum 专用的 version-aware `asset_usages`。Block 可从
  pending 或 clean 进入不可公开的 quarantine，外部删除不持有 DB lock，并有 retry/dead-letter。自动
  scan、跨所有业务的 usage 和 orphan GC worker 仍为 `Planned`，不能因删除 worker 或主题/评论链路
  存在声称完整 OSS 产品链路。
...
- 软删除将 active usage 标记为 `target_deleted` 并设置 30 天后才可进入 GC 候选，不删除 OSS object；
  staff restore 在同一锁序重新解析 canonical source 并重验 owner/clean 后 rebind。实际 GC 还要求无
  active usage、无 legal hold 且由未来 worker 执行，失败发布不会留下 active usage。
