...
- 主题最多 8 张、评论最多 4 张 clean 图片；canonical source 只接受带非空 alt 的
...
- Avatar/banner、主题/评论与推广素材已完成 owner 上传或 clean reference、业务事务内 binding、Forum
  draft reference 和解绑 grace。Retention-aware GC 已有实现与测试，但默认 rollout flag 关闭；课评/私信
  仍没有 binding，scanner/变体/EXIF 尚未完成，因此这些子链路仍不等于已启用的媒体产品闭环。
- 聚合搜索已有六类 typed 结果、有效 type filter、独立 Web 综合结果页与全局搜索入口；高亮由 owner
  ...
  局部失败状态已完成。
- 推广使用 media-owned 非版本化 binding，并在替换/归档时进入 30 天 grace；仍缺匿名 clean 图片交付。
  这些缺口不影响无图片卡片和登录用户通过 media 授权 URL 显示 clean asset。效果数据已按日聚合，
  但不等于商业广告归因或跨域画像。
...
安全状态: intent -> uploaded/pending -> clean
                                  \-> quarantined -> blocked
                                         |             |
                                  durable retry     object removed

绑定状态: unbound <-> bound -> gc_eligible -> garbage_collected
```

- 当前数据库有 intent、`pending/clean/quarantined/blocked`、moderation evidence、durable provider
  deletion job、profile/推广非版本化 `asset_bindings`，以及 Forum 专用的 version-aware
  `asset_usages` 和 `draft_asset_references`。Block 可从 pending 或 clean 进入不可公开的 quarantine，
  外部删除不持有 DB lock，并有 retry/dead-letter。通用 GC 代码只选择 `cleaned_at` 已满 30 天、无 live
  usage/binding/draft reference、无 future grace、无 active operational hold 的 clean object，并在锁后
  重验；`pending` 永不因年龄自动删除。GC 与账号 purge system enqueue 默认 rollout-gated，代码合并或
  部署不等于目标环境已启用。自动 scan、课评/DM binding 与覆盖全部业务 surface 的 usage 仍为
  `Planned`，不能据此声称完整 OSS 产品链路。
...
- 推荐 private bucket；staff pending preview 已使用一次性、可审计、同源 bounded proxy，owner 仍只使用
  status/现有受控 URL 能力；公开内容只引用 clean asset。最终
...
- Forum draft 保存本人 pending/clean asset 时同步 exact draft reference；发布时仍要重验 clean 并切换为
  version-aware usage。Migration `0057` 在 backfill 前安装 profile/promotion/draft source trigger，为旧
...
- 目标态是自动扫描处理正常图片、人工队列只处理命中/失败/举报；在 scanner 未交付的当前阶段，raster
  image 必须由人工可信预览后批准，file/PDF 不允许人工绕过扫描直接放行。
