...
>
> 最近核验：2026-07-15，`contract/openapi.yaml`、migrations `0061`、`0065` 与 owner-domain tests
...
- 首页、论坛、Profile 与收藏使用同一 query-key/viewer-state 家族；vote/bookmark mutation 会先取消
  相关请求、快照所有受影响 cache 并乐观更新，失败按 mutation generation 回滚，settle 后回源校正。
  SSE 只对当前账号做 typed 定向失效，不把事件载荷当 canonical 内容。
- 首页 feed 条件同步到可复制的 URL；主题分享始终生成无 query/tracking 的 canonical detail URL，优先
  使用系统 share，降级为剪贴板。卡片导航与互动、分享、图片灯箱保持独立可聚焦控件。
...
- 课评有发布、编辑、点赞/取消、举报和审核基础；公开列表与精确详情均回表重验当前可见性，并返回
  `viewerLiked/canEdit/canReport`。Web/Flutter 按这些字段提供取消点赞、本人编辑、非本人举报和搜索
  精确 deep link。历史导入的 `reviewer_avatar` 是不可信远程地址，不进入公开或管理 DTO；Reviews 尚未
  接入 Media owner 的 typed avatar projection 时统一返回 null，客户端使用作者 handle fallback。
...
- Web 同时把最小 typed payload 保存为 account-scoped IndexedDB 恢复副本，最多保留 7 天；只保存正文
  字段和 attachment asset id，不保存 Delivery URL。云端不可用时可恢复，若本机副本更新则让用户显式
  选择版本；发布、登出或切换账号会清除对应副本。
...
- Web 全局搜索框在输入稳定 400ms 后请求，React Query cancellation signal 会终止旧请求；新词等待期
  不展示旧结果。上下方向键在结果间移动焦点，Enter 打开焦点项或从输入进入综合页，Escape 关闭；
  课评结果进入精确 review detail，不降级到可能找不到命中对象的课程第一页。
...
- 首页已移除固定摘要、按 index 伪造徽章和无行为的筛选/频道按钮，并已接 canonical 摘要、列表级
  viewer state、跨 cache 乐观互动、canonical 分享与用户 following；`hot` 仍是热榜而不是个性化
  recommended 模型。
...
- 聚合搜索已有六类 typed 结果、有效 type filter、独立 Web 综合结果页与全局搜索入口；高亮由 owner
  ...
  仍缺拼音/别名，以及 transactional outbox 驱动的索引可靠更新。单类 cursor、all 页“查看更多”、
  局部失败、全局框 debounce/cancellation/键盘导航和精确课评 deep link 已完成。
...
- Feed/论坛/Profile 列表的首张 clean attachment 与详情 Markdown 图片可通过独立按钮打开共享灯箱；
  卡片导航链接不包裹按钮，避免 `a > button`。列表契约只交付首张 attachment，客户端不得伪造多图；
  详情页按 canonical attachment 顺序切换多图，链接图片保持链接语义且不叠加灯箱。灯箱支持关闭按钮、
  空白区、Escape、左右方向键、可见计数、按钮缩放和当前 Delivery URL 下载，并在关闭后恢复触发焦点。
...
生成的测试证书，不调用公网，也不放宽生产策略。网络抓取边界和 Web opt-in 文字预览均为 `Current`：
编辑器只有在用户打开对话框、输入标准 HTTPS URL 并点击“安全预览”后才调用站内 `/onebox`；浏览器
不直连目标站、不渲染响应中的 `imageUrl`，输入不进入 route/query 或本地持久存储。用户再次确认后才把
服务端规范化 URL 作为普通 Markdown 链接插入正文。当前产品明确不展示远程预览图，因此 media proxy
不是上线前置；若未来要显示图片，必须先接平台 asset/proxy。
...
- Web 的 IndexedDB 恢复副本是对云端 CAS 草稿的有界容灾，不是 canonical storage 或离线发布队列；
  断网时可以跨刷新保留输入，但创建/编辑正文和绑定媒体仍必须重新在线通过服务端验证，产品不能把它
  描述为 offline-first。
...
导航栏加载本人头像事实与 `thumb_256` delivery 期间使用不含姓名首字母的中性骨架；刷新短期 URL 时继续
显示上一次已成功加载的图片，只有确认无头像或加载失败且无旧图时才显示文字 fallback。
...
- Feed 多图需要扩展列表 attachment projection，还是只在打开灯箱时读取详情；决策必须包含带宽、短期
  Delivery 授权、缓存和被删除/blocked asset 的刷新语义，当前单图列表不能被宣传成 gallery。
- 敏感内容分类：分类类别、作者/审核者标记权、默认遮罩与用户 reveal 偏好、搜索/通知摘要、举报/申诉/
  审计、未成年人边界及保留期限。它不是 watched-word 或恶意文件扫描的同义词，字段和政策批准前不实现。
- 是否增加多符号 reaction；若批准，需要独立 typed 状态、撤销/计数、通知聚合、滥用控制和跨端契约，
  不能把现有正负 vote 的整数语义改名后复用。
- 班级/社团 Space 的真实性与成员来源、角色/名单隐私、毕业/退出生命周期、moderation、创建配额，及其
  与 Board、课程/教学班 owner 的边界。当前 Board 无 kind 或成员角色，决策前不创建半套成员表。
