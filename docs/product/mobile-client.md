...
>
> 最近核验：2026-07-15，selection teaching-class 契约、本机 schema 迁移与 Flutter 分页/冲突回归
...
| 选课与课表 | `Partial` | 学期/年级/专业/性质、关键词、稳定游标分页与 freshness 时钟已消费 canonical offering API；本机课表以 `offeringId` 区分同课平行教学班，schema v2 可保守迁移到 v3，冲突以显式 `weekNumbers/weeksUnknown` 判定。页面仍明示它是 client-owned 排课而非官方选课；跨设备同步仍待决，也缺真实选择→冲突 Android/iOS 设备 journey |
...
| 私信 | `Partial` | canonical 1:1、incoming/sent 请求箱、接受/拒绝/撤回/举报、分页/搜索、未读、mute、archive/delete/recover 和 resume 回源已实现；尚未消费 DM SSE，附件/request expiry/typing/presence/retention 仍是后端产品缺口，兼容头像与设备 master-detail 也未验证 |
...
节次统一按平台 API 的 1-based inclusive `startSlot..endSlot` 展示，课表画布至少容纳 1–13；冲突依据同一天
