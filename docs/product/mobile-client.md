...
>
> 最近核验：2026-07-14，Flutter 真实 API journeys、widget/contract tests 与 Web/OpenAPI 对齐审计
...
自动化/设备证据同时存在时，移动端行为才可标为 `Current`。2026-07-14 审计后 19 项均已有真实实现，
...
| 选课与课表 | `Partial` | 学期/年级/专业/性质、更新时间、检索、课程级本机课表、1-based 节次冲突及 environment+account+calendar 隔离已实现；后端仍缺 teaching-class identity/完整周次/状态，跨设备同步仍待决，并且没有真实选择→冲突设备 journey |
