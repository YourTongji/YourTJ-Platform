...
>
> 最近核验：2026-07-17，wallet owner-key/outcome 契约、exact signing 与 pending recovery
...
| 课程与评课 | `Partial` | 课程浏览/检索/详情、AI 摘要、关联课程以及课评列表、创作、点赞和举报已接真实 API；legacy reviewer avatar 不再由服务端投影，客户端也只接受当前环境 API/CDN 精确 origin，typed Media avatar 落地前显示 handle fallback。`Review` 契约仍缺 viewer-liked/can-edit，移动端不能可靠呈现取消点赞或本人编辑状态，也未消费已有 edit route，相关 integration/widget 覆盖仍不足 |
| 选课与课表 | `Partial` | 学期/年级/专业/性质、关键词、空闲时段、稳定游标分页与 freshness 时钟已消费 calendar-scoped canonical offering API；本机课表以 `offeringId` 区分同课平行教学班，兼容已发布本机 envelope，冲突以显式 `weekNumbers/weeksUnknown` 判定且 confirmed 不可覆盖。1–20 节保留，课表可导出/分享不含账号资料的 JSON。页面仍明示它是 client-owned 排课而非官方选课；跨设备同步仍待决，也缺真实选择→冲突 Android/iOS 设备 journey |
...
| 积分 | `Partial` | 钱包、ledger/verify、任务、商品/订单、tip/escrow、旧钱包 claim、account-scoped Ed25519 seed 与共享 exact-byte 向量已接通；首次登记和每次签名前都读取 required owner `activePublicKey`，只允许明确 null 后生成或与本机实际 signer 精确匹配，字段缺失/错号/错 key 均 fail closed。客户端校验 intent 的 account/action/key/id/expiry/idempotency envelope 后签 raw exact bytes，发送前持久保存 operation digest + intent id；未知响应只按与消费事务行锁互斥的 owner outcome 判定 committed/expired，pending/查询失败继续阻止重放。仍缺 key rotation/recovery、真实断网/长事务 integration、Keychain/Keystore/backup 真机验证及本机记录最终生命周期演练 |
...
旧选课客户端展示了教学班选择、单双周冲突、换班和变更检测等有价值的产品方向。当前平台已完成
teaching-class 读契约：`offeringId` 绑定具体教学班和学期，时段显式携带周次集合与未知标记，Web/Flutter
都按该 identity 建立 client-owned 课表。上游当前不提供可信的选中结果或完整生命周期，所以 UI 仍不得声称已官方选课，
也不得承诺停开、换班或时间变更监测。

节次统一按平台 API 的 1-based inclusive `startSlot..endSlot` 展示，课表画布按实际数据扩展并完整容纳 1–20；冲突依据同一天
节次区间与 `weekNumbers` 交集判断。`weeksUnknown=true` 或集合缺失时只能提示“周次未知并按可能冲突处理”，
不能当作全周或无冲突。教学班课表存在本机，必须以 `environment + accountId + calendarId` 分区；匿名数据
单独分区，登录后不自动并入账号。跨设备同步属于 `Decision needed`，没有服务端 owner 前不发明
移动专用云表。本机 JSON 导出/系统分享不包含 account id、token 或 environment credential，也不自动导入
其他账号或学期。
...
- 生成前必须先读取当前账号 owner `/wallet`：只有 required `activePublicKey` 明确为 null 才可创建 seed；已有
  key 时只允许本机 exact match 的幂等确认。字段缺失、wallet account 错号、非规范 key 或本机 key 缺失
  都不能生成 orphan replacement。bind body 的 account id、bearer token 与 session generation 必须来自同一
  捕获快照；即使 final check 后、Dio dispatch 前切换账号，也不能把新账号 token 配到旧账号 key/body。
  每次 intent 前重复同一精确匹配，并绑定 session generation。
...
- 钱包写入先请求服务端 signing intent，签名前验证 exact 10-field canonical envelope、规范请求
  SHA-256、version/account/action/key/intent/idempotency/expiry、同次 owner wallet 余额与页面已展示的
  product/task/purchase snapshot，并按 action 精确绑定 ledger 金额、收方和 metadata；不对 opaque
  proof 盲签。随后只对原始 exact bytes 签名，不重组或“等价序列化”payload。signer 必须由本次
  实际使用的 seed 派生公钥并再次与 expected key 比较，防止校验/签名间换钥。
- 无响应/5xx 后只通过固定 URL POST body 查询 owner-visible intent outcome，不把 intent id 放进
  access-log path。服务端状态读取用行锁等待并发消费事务终态：
  `committed` 才确认已提交，`expired` 才证明已过期且未消费，`pending`、超时或错误一律保留本机记录并
  fail closed；客户端不能再用某一时刻查不到 ledger/task/purchase 猜测未提交。
- pending schema 只保存 operation digest、action、intent id/expiry 并严格拒绝未知字段。schema 升级只能在
  新记录完整覆盖旧 submitted 记录后删除旧 key；缺少 intent id、因而无法查询 outcome 的旧记录必须原样保留
  并阻止新签名，不能为了迁移成功而误清未知结果。
...
3. **公开与普通用户旅程**：匿名浏览、身份、首页、论坛、课程/评课、教学班级本机课表、搜索、资料/社交、
...
以下事实不阻止 Web-parity 客户端交付，但必须保持显式状态：官方选课/教学班变更监测、系统后台 push、
