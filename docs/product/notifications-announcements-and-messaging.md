...
>
> 最近核验：2026-07-15，migrations `0054`/`0068`、Web/Flutter announcement gates 与 notification/DM clients
...
- 通知列表支持有界 cursor pagination、数据库 unread filter、逐条/批量/全部已读；Web 有全局未读
  角标、全部/未读筛选、加载更多，以及内容/私信等已知事件的安全站内 target。加载态 skeleton 与真实
  通知卡结构一致；普通通知按用户本地自然日分组，治理通知仍保留独立证据边界。
...
- 普通消息可携带 client-generated message UUID；服务端按 sender + UUID 串行并唯一约束，同 UUID/
  conversation/body 返回同一消息，不同内容冲突且不重复生成通知。Web 与 Flutter 先显示发送中气泡，
  失败后保留正文并用原 UUID 明示重试或丢弃，不执行无身份的自动重发。
...
- `clientMessageId` 只用于一次普通消息的幂等重放，保留期跟随该消息；它不是 installation/device id，
  不得复用于账号关联、分析或跨消息 dedupe。省略该字段仍兼容旧客户端，但不能获得失败重试防重复保证。
...
- client message UUID 是消息行的内部幂等元数据，正文删除时一并删除；公开响应、通知 payload、日志和
  owner 画像不暴露该值。
...
- 私信 policy、request、block、sanction、participant 权限、read pointer，以及相同/冲突 client message
  UUID 的重放有数据库集成负向测试；一次成功发送只产生一条消息和一份通知事实。
