...
该实现有四项 `Partial/P0` 偏差：

- TL2/TL3 注释要求 active-days，但 promotion SQL 没有检查 active days。
- Demotion 先执行 TL3→TL2，再执行 TL2→TL1，原 TL3 可能在同一次扫描下降两级，与“降一级”规则冲突。
- Board 已有 `is_locked` / `min_trust_to_post` 字段，但 create-thread 路径未执行这两个 policy。
- Staff 内容达到普通举报阈值时应自动隐藏，还是转入独立复核队列，尚无明确政策；当前行为必须
  在 policy 与层级测试完成前视为 P0 治理缺口。

修复前不得把 trust level 宣称为完整信誉体系，也不能依靠未执行的 board 字段做授权承诺。阈值和
promotion 条件应改成 versioned/可测试 policy，后台只能修改被产品批准且有审计的参数。
