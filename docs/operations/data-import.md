...
>
> 最近核验：2026-07-17，migration `0071`、16-table real-snapshot import、历史评分与 search reconciliation

本 runbook 覆盖 Cloudflare D1 选课快照中 13 张 selection 表和 3 张 legacy course 表的首次/恢复导入，
再物化 `courses.*` 与 `selection.*`。它恢复可公开的历史评分聚合，但不是完整生产数据迁移，也不迁移旧评课
正文、作者、点赞/举报、身份或钱包归属。
...
```text
D1 snapshot
  ├─ 13 selection tables -> selection.pk_* raw
  └─ teachers/courses/course_aliases -> courses.pk_legacy_* raw
              -> courses.* catalogue + public historical aggregate
              -> selection.* teaching classes + scoped historical aggregate
```
...
- 旧 course review count/average 仅在 legacy row 能唯一映射到当前 catalogue owner 时进入公开 aggregate；
  无法证明的映射留在 raw 层。`reviews.*` 正文、身份与互动导入仍需要独立 identity/course mapping、privacy
  和 moderation 决策，不属于本流程。
...
- 两份 materializer 在首个写操作前都会锁定 16 张 raw tables，并要求同一 snapshot hash 绑定的
  `selection.import_runs`（13 张 selection 表）和 `courses.legacy_import_runs`（3 张 legacy course 表）中
  ...
  以及 legacy teachers/courses/aliases 任一核心表为空，或无学期/课号教学班、孤立/越界时段都会 fail closed。
...
# 2. 原子导入 16 张 raw tables，并落两份同 snapshot audit 和一份 manifest
...
`--compare-manifest` 是默认必需的获批基线：schemaVersion/sourceDatabase、snapshot hash 与 16 张表完整
count key 必须有效，selection 或 legacy-course 核心表任何下降都会在写 manifest/数据库前拒绝。确认上游
确有合法下降时，必须显式附加
...
mode、reason、baseline hash/counts 会同时进入两份 import audit，materializer 会按各自表集再次校验。
...
两份 audit 同事务提交才表示 PostgreSQL 导入成功。脚本当前导入 13 张 `selection.pk_*` 和 3 张
`courses.pk_legacy_*` raw table。精确映射和参数以
...
- COPY、row-count 验证或任一 import audit 写入失败时，导入事务会整体回滚；不要跳过 empty-table guard
...
- 已提交的共享恢复环境只能按其已审批的数据库备份恢复流程回滚；不得直接 `TRUNCATE` raw 表。先恢复
  两组 raw table 的同一一致快照，再重跑两份幂等 materialization，最后重建 search/cache 并完成本节验证。
...
真实上游安排行可以无身份前缀，也可以使用 `教师名(工号)`、`(工号)` 或多身份前缀。单一身份在当前教学班
中能唯一解析时随时段保留；多身份前缀仍可产生排课事实，但教师归属为空，不能从 raw row 顺序猜一位教师。
...
### Migration `0071` 升级与回退意图

`0071` 新建三张 Courses-owned legacy raw table、与 selection import snapshot hash 绑定的 audit、公开历史评分
列和 search projection readiness。它不从旧 `reviews.*` 反向推断来源，也不导入正文/身份。升级后既有
selection raw 环境缺少三张 legacy source，两个 materializer 会 fail closed；operator 必须在隔离/维护窗口
从同一获批 D1 快照重建全部 16 张 raw table，不能只手工补三张表或伪造 audit。

第一次恢复 aggregate 前还必须证明现有 `courses.courses.review_count/review_avg` 只来自当前 PostgreSQL
community review，而没有提前包含同一批 D1 aggregate。Materializer 会把首次看到的既有值视为 community
部分并另加 legacy 部分；若环境曾手工导入相同旧评课或预灌同一 aggregate，直接运行会重复计数。此时停止
rollout，先建立可审计的 source-id 去重/回填 migration；不能通过手调 average 或删除正文掩盖重叠来源。

升级工具生成的 manifest 由 13 个 count key 扩为 16 个，旧 manifest 不能直接作为新版 comparison baseline。
先在受控环境用旧 manifest 与源备份人工核对，再以 `--approve-unbaselined-snapshot` 建立一次新版可信基线并
记录理由；后续恢复重新使用普通 comparison。应用回退应保留 raw/audit/aggregate/readiness schema 并
forward-fix，不能 drop 列或把旧 aggregate 当成 community review 重复累加。
...
Migration `0071` 后 catalogue 与 selection 各有 PostgreSQL readiness row。物化会推进 source generation 并
置 stale；full reindex 只有在 generation/row count 仍一致时才能标 ready。API 启动会同步比较数据库状态与
Meilisearch live document count，随后每分钟复核一次；空索引或外部清空会触发重建，未 ready 的关键词查询
返回 503。部署验证必须同时查看两条 readiness row 和两个 index 的 document count；不能只因 HTTP 返回
空数组就判断“没有匹配课程”。clear→add 的可见空窗仍存在，readiness 只负责 fail closed，不提供原子 alias。
...
- 同 snapshot 的两份 validated import audit，其 source/target counts 与锁定后的 16 张 live raw tables 完全
  一致，并包含各自有效 completeness approval；主动用缺失/旧格式 audit、核心表空值、未经批准的 count
  decrease 和改动一张 live raw 表计数的 fixture 验证两份 materializer 都在任何写入前拒绝执行。
...
- 公开历史 aggregate 的总样本数不超过 legacy source，未唯一映射的差额有记录；教学班分别抽查 teacher、
  course fallback 和 none 三种 scope。重复运行两份 materializer 后 legacy 与 community 部分都不得重复累加。
- 启动前删除/清空测试 Meilisearch index，确认 startup reconciliation 恢复 catalogue/selection document
  count；人为置 stale 或在重建中查询应得到 503，不得得到误导性的零结果。

### 2026-07-17 本地隔离复验

本地隔离验证使用仓库外已有的 `jcourse-db-backup` SQLite 快照；该文件没有随附可验证导出时间的签名
manifest，因此以下数字是开发复验证据，不替代 operator 的正式基线。16 张表共 184,351 行：13 张
selection 表 141,536 行，其中 `coursedetail=20,834`、`teacher=34,745`、
`teacher_timeslots=20,160`、`majorandcourse=64,521`；3 张 legacy course 表 42,815 行，其中
`teachers=4,502`、`courses=9,518`、`course_aliases=28,795`。

物化后有 4,816 个 catalogue course、20,834 个 offering、64,521 条专业绑定和 40,530 条去重时段；
`scheduleUnknown=3`、offering `weeksUnknown=8`、时段 `weeksUnknown=1`、`locationUnknown=14`，非法节次和
重复时段事实均为 0，最大单 offering 51 条时段。8,434 个 legacy 评分样本中，8,206 个经唯一课程映射进入
1,034 个 catalogue course；6,743 个 teaching class 获得历史参考，其中 3,772 个为 teacher scope、2,971 个
为 course fallback。第二次完整物化的行数和 aggregate 不变。

这些数字是本次 operator 按上述命令在仓库外隔离环境记录的可复现证据，不是 CI artifact、production telemetry
或线上 SLO 证明。

`fetchlog` 最新上游时间为 2026-06-11；无论本地导入发生在何时，`/selection/latest-update` 都必须据该
source clock 判断 freshness，不能用 import time 掩盖陈旧数据。正式发布仍需按本节要求补公开 HTTP 抽样、
staging Meilisearch 恢复和 preview journey 证据。
...
2026-07-17 本地复验中 catalogue 约 0.7 秒、selection fresh run 约 41.3 秒（warm rerun 约 32 秒），读者
只会看到旧完整快照或新完整快照；规模和复杂度尚不支持引入删除检测、跨表增量
...
Migration `0071` 只恢复课程级公开 count/average，不创建 `reviews.reviews` 行。
