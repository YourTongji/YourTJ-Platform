...
function ReviewCard({ review, courseId }: { review: Review; courseId: string }) {
  ...
  const report = useMutation({
    mutationFn: () => api.reportReview(review.id ?? "", "other"),
    onSuccess: () => toast.success("举报已进入审核队列"),
    ...
    <Card>
      <CardContent className="p-4">
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={() => report.mutate()} disabled={report.isPending}>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>评分必填，其余字段可选。发布接口带 Idempotency-Key 防重复提交。</CardDescription>
      </CardHeader>
